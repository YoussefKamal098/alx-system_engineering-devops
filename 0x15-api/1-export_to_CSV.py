...
online REST API service (https://jsonplaceholder.typicode.com) to a CSV file.
...
Description:
    ...
    - It filters the tasks to get only those belonging to
        the specified user.
    - It writes the user's tasks to a CSV file.

Example:
    python script_name.py 1

The generated CSV file will be named with the current date and time.
"""
...
def export_to_csv(
    ...
    """
    Export tasks for the specified user to a CSV file.
    ...
    with open(f"{filename}.csv", "w") as file:
        # use this if you want to use the csv module
    ...
    print(f"Tasks have been exported to {filename}.csv")
...
def main():
    ...
    # Export tasks to CSV
    export_to_csv(userId, str(userId))
